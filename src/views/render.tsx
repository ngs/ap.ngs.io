...
const htmlResponse = (jsx: Child): Response => {
  const html = '<!DOCTYPE html>' + String(jsx);
...
export async function handleHomePage(env: Env): Promise<Response> {
  ...
  return htmlResponse(<HomePage domain={env.DOMAIN} accounts={accounts} />);
}
  ...
  const users = await Promise.all((results || []).map(async r => {
    ...
    if (actorJson?.preferredUsername) {
      return {
        ...
        iconUrl: actorJson.icon?.url || (typeof actorJson.icon === 'string' ? actorJson.icon : undefined),
        domain,
      };
    }
